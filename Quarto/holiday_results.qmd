...
- No adjusting for study leads to a large bias 
When using random intercept models bias reduces with stidy size, to become almost zero for study size 1000.
...
The final results show increasing bias with the number of studies, when study is not adjusted for, or random intercept models are used. This is the case for random intercept models when study size is 50. This goes against intuition.

## A closer look at random intercept models

Focusing on scenarios with correlatied intercepts and predictors, removing model that does not adjust for study, and removing the smallest studies.

```{r, fig.height = 6}
sumaries_focus <- summaries |> 
  filter(int_pred_corr == 0.5, model != "Not adjusting for study", study_sample_size_train !=50, ICC !=0)
sumaries_focus |> 
  filter(int_pred_corr == 0.5, model != "Not adjusting for study", study_sample_size_train !=50) |> 
  plot_results_by_model(CI = TRUE) + ylab("Percentage bias in beta")

```

To understand the differences we see we can compare the parameters used in the models:
...
When r-squared is 0.4 we see greater bias as beta x is smaller compared to sigma u. If we plot the values of bias, rather than as a percentage of beta, we see no difference with changing r-squared.
...
summaries |> 
  filter(int_pred_corr == 0.5, model != "Not adjusting for study", ICC !=0) |> 
...
summaries_calib_slope |> 
  filter(model != "Not adjusting for study", ICC !=0) |> 
...
When there is level 2 endogeneity we see bias in beta estimates. This bias does not change with the number of studies but it does change with the number of people per study. This is consistent with the literature, which shows (Castelano 2014) (analytically shown for generalised least squared estimates in Maddala 1971). Intuitively the random intercept estimate of beta can be seen as pooling between cluster and within cluster info in beta. Bias arises from correlation between the between cluster varaition in x and intercepts. This leads to bias in the between cluster estimates of beta. When cluster size is large, little weight is given to between cluster estiamtes and the unbiased within cluster estimates dominate. 

On reflection (and reading RHS section 3.8 p158), the data generating model chosen for my simulations favours fixed effects models due to predictors varying within clusters but not between clusters. Fixed effects models only use within cluster info, this is large in my data as this is where the variation in predictors occurs. If there is more between cluster variation in predictors (and within cluster and between cluster effects are identical/similar) then the random intercept model can have lower mean squared error as it can make use of between cluster info. This would occur if clusters were homogeneous but different, for example if we have studies recruited based on intellectual disability with little variation within clusters. I also assume constant within and between cluster effects. If there is a differing between cluster effect of a predictor (how could this occur - could be driven by slection eg. clinical samples have lower IQ, but also worse outcomes because of other unmeasured factors which are lower in clinical samples)

The impact of bias arising form level 2 endogeneity on mean squared error for betas, and on overall R-squared is small. There is however an impact on calibration slope. This is because estimates of betas are biased upwards. This leads to more extreme predictions for people with extreme values of x, leading to calibration slope less than 1 (note to self - calibration has observed as outcome and predicted as predictor). 
...
To many dimensions to think about. What about bias variance. Think about searching for daylight between FE and RE models. What are the impliacitons for individual level prediction - does this fundamentally focus on within cluster effects? 

...
-   This can lead to heterogeneity in calibration-in-the large

-   Knock on effect on r-squared

-   Having a moderate sample to estimate the intercept can overcome any issues from poor estimation of study intercepts - even when the intercept is not included in the model

-   Calibration slope was good except when sample size was small

```{r, echo = FALSE}
# Processing database results to give summaries and ns
data <- dbGetQuery(db,
           "SELECT n_studies, ICC, R2, study_sample_size_train , int_pred_corr, model, est, tau2, sigma_e, sigma_u, intercept_est_sample_size
            FROM sim_results_v1
...
scenarios <- dbGetQuery(db,
            ...
            FROM sim_results_v1
            WHERE sim_name = 'BatchAug2023'  AND metric = 'calib_itl' AND"
)

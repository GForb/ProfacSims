box_plot_cluster_x <- function(data) {
  plot_data <- data |>
    dplyr::mutate(
       ...
       b_w_ratio = factor(b_w_ratio, levels = c("1", "1.25", "2"), labels = c("Between effects = Within Effects","Between effects 25% greater than within effects", "Between effects double Within effects")))
